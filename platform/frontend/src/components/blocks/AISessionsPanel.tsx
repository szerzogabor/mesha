...
import { BlocksSession, BlocksMessage, AIExecutionState, LinkedPullRequest } from "@/types";
...
function SessionPRCard({ session }: { session: BlocksSession }) {
  const pr = session.linkedPullRequest;
  const url = pr?.htmlUrl ?? session.prUrl;
  if (!url) return null;

  const displayPr: LinkedPullRequest = pr ?? {
    id: session.id,
    htmlUrl: url,
    githubPrNumber: session.prNumber ?? undefined,
    sourceBranch: session.branchName ?? undefined,
  };
  ...
  return (
    ...
    >
      ...
      <span
        className={`mt-0.5 shrink-0 inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-medium ${prStateBadgeClass(displayPr)}`}
      >
        {prStateLabel(displayPr)}
      </span>
      <span className="flex-1 min-w-0">
        <span className="block font-medium text-text-primary truncate">
          {displayPr.title
            ? displayPr.title
            : displayPr.sourceBranch
              ? displayPr.sourceBranch
              : `Pull Request${displayPr.githubPrNumber ? ` #${displayPr.githubPrNumber}` : ""}`}
        </span>
        <span className="flex items-center gap-2 mt-0.5 text-text-tertiary">
          {displayPr.githubPrNumber && <span>#{displayPr.githubPrNumber}</span>}
          {displayPr.sourceBranch && (
            <span className="truncate max-w-[120px] font-mono">{displayPr.sourceBranch}</span>
          )}
          {displayPr.checksStatus && (
            <span className="flex items-center gap-0.5">
              {displayPr.checksStatus === "success" && (
                <svg className="h-3 w-3 text-green-600" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                </svg>
              )}
              {displayPr.checksStatus === "failure" && (
                <svg className="h-3 w-3 text-red-600" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                </svg>
              )}
              {displayPr.checksStatus !== "success" && displayPr.checksStatus !== "failure" && (
                <svg className="h-3 w-3 text-yellow-500 animate-spin" viewBox="0 0 24 24" fill="none">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                </svg>
              )}
              <span>{displayPr.checksStatus}</span>
            </span>
          )}
        </span>
      </span>
...
function SessionConversation({
  ...
  return (
    <div className="space-y-3 pt-2">
      ...
      {/* PR card - attached to this session */}
      <SessionPRCard session={session} />
        ...
        {(session.linkedPullRequest || session.prUrl) && (
          <span className={`shrink-0 inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-medium ${
            session.linkedPullRequest
              ? prStateBadgeClass(session.linkedPullRequest)
              : "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
          }`}>
            {session.linkedPullRequest
              ? `PR #${session.linkedPullRequest.githubPrNumber ?? ""}`
              : session.prNumber
                ? `PR #${session.prNumber}`
                : "PR"}
          </span>
