...
// Close stops background replenishment, waits for in-flight pre-warm creates,
// and destroys all idle VMs. It does not track already allocated VMs.
...
func (p *WarmPool[T]) ensureRunning() error {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.closing {
		return errors.New("warm VM pool is closing")
...
func (p *WarmPool[T]) createIdleVM(parent context.Context) {
	...
	if err != nil {
		...
		shouldRetry := p.started && !p.closing && parent.Err() == nil
		retryAfter := p.config.ReplenishInterval
		p.mu.Unlock()
		if shouldRetry {
			p.retryReplenish(parent, retryAfter)
		}
		return
	}
...
func (p *WarmPool[T]) retryReplenish(ctx context.Context, delay time.Duration) {
	p.wg.Add(1)
	go func() {
		defer p.wg.Done()
