...
func (f *testVMFactory) CreateVM(ctx context.Context) (testVM, error) {
	if f.createStart != nil {
		f.startOnce.Do(func() { close(f.createStart) })
	}
	if f.delay > 0 {
		select {
		case <-ctx.Done():
...
func (f *testVMFactory) destroyedCount() int {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func TestWarmPoolReleaseDestroysDirtyVMAndReplenishes(t *testing.T) {
	t.Parallel()

	factory := &testVMFactory{}
	pool, err := NewWarmPool[testVM](WarmPoolConfig{DesiredIdle: 1}, factory)
	if err != nil {
		t.Fatal(err)
	}
	defer closePool(t, pool)
	...
	waitFor(t, func() bool { return pool.IdleCount() == 1 }, "initial pre-warm")

	vm, err := pool.Allocate(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if err := pool.Release(context.Background(), vm, false); err != nil {
		t.Fatal(err)
	}

	waitFor(t, func() bool { return pool.IdleCount() == 1 }, "replacement VM after dirty release")
	if got := factory.destroyedCount(); got != 1 {
		t.Fatalf("expected dirty VM to be destroyed once, got %d destroys", got)
	}
}

func TestWarmPoolCloseCancelsInFlightOnDemandAllocation(t *testing.T) {
	t.Parallel()

	createStart := make(chan struct{})
	factory := &testVMFactory{delay: time.Second, createStart: createStart}
	pool, err := NewWarmPool[testVM](WarmPoolConfig{DesiredIdle: 0}, factory)
	if err != nil {
		t.Fatal(err)
	}
	if err := pool.Start(context.Background()); err != nil {
	...
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for on-demand create to start")
	}
	closePool(t, pool)

	select {
	case err := <-allocateErr:
		if err == nil {
			t.Fatal("expected allocation to fail after pool close")
		}
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for allocation to observe pool close")
	}
}
