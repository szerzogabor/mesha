...
trap 'umount "$ROOTFS/dev" 2>/dev/null || true; umount "$ROOTFS/sys" 2>/dev/null || true; umount "$ROOTFS/proc" 2>/dev/null || true' EXIT
...
ENGINE_ARGS=(run --rm --privileged -v "$STAGING_DIR:/rootfs" -v "$OUT_DIR/build-rootfs-inside-container.sh:/build-rootfs.sh:ro" "alpine:$ALPINE_VERSION" /build-rootfs.sh "$ALPINE_REPOSITORY" "$ALPINE_VERSION" "$CLAUDE_CODE_VERSION" "$BUILD_EPOCH")
if [[ "$ENGINE" == "podman" ]]; then
  ENGINE_ARGS=(run --rm --privileged -v "$STAGING_DIR:/rootfs:Z" -v "$OUT_DIR/build-rootfs-inside-container.sh:/build-rootfs.sh:ro,Z" "alpine:$ALPINE_VERSION" /build-rootfs.sh "$ALPINE_REPOSITORY" "$ALPINE_VERSION" "$CLAUDE_CODE_VERSION" "$BUILD_EPOCH")
fi
...
mkfs.ext4 -q -F -d "$STAGING_DIR" -E root_owner=0:0,lazy_itable_init=0,lazy_journal_init=0 "$IMAGE_PATH"
